...
class GenLazyNativeFuncDefinition:
    ...
    def __call__(self, func: NativeFunction) -> List[str]:
        ...
        if schema.name.name.inplace and symbole_str[-1] == "_":
            symbole_str = symbole_str[:-1]

        node_str = f"""torch::lazy::NodePtr node = nullptr;
        if (FLAGS_torch_lazy_reuse_ir) {{
            TORCH_LAZY_TIMED("LazyNativeFunctions::ReuseNode");
            node = torch::lazy::ReuseNode<ir::ops::{schema.node_name}>(
                    torch::lazy::OpKind({symbole_str}),
                    {node_ctor_input_str});
        }} 
        if (!node) {{
            TORCH_LAZY_TIMED("LazyNativeFunctions::ComputeShapeAndMakeNode");
                {meta_str}
            {{
            TORCH_LAZY_TIMED("LazyNativeFunctions::MakeNode");
